...
dusty.module.iem = (function() {
		...
		handleIemData: function(data) {
			...
			if (data.messages.length > 0) {
				...
				if (!isNaN(tmpLastSeqNum)) {
					lastSeqNum = tmpLastSeqNum;	// TODO declare this
				}
			}
		...
		render: function() {
			...
			if (this.state.messages) {
				messages = this.state.messages.map(function(m) {
					var formattedMessage = _formatMessage(m);

					return (
						React.createElement(IemMessage, {message: formattedMessage})
					);
				});
			}
	...
	var _formatMessage = function(m) {
		if (m == null || m.product_id == null) { return; }
		...
		var li = $('<li/>', { 'class': code }).addClass('shadow');
		$('#iem > ol').prepend(
				li.append($('<div/>', { 'class': 'iemTimestamp', 'text': '[' + moment(m.ts).format('HH:mm') + 'Z]' }))
				.append($('<div/>', { 'class': 'iemMessage', 'html': m.message }))
		);
