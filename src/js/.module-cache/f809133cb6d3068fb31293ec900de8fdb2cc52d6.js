...
dusty.module.iem = (function() {
	var _init = function() {
		...
		React.render(React.createElement(IemDisplay, null), document.getElementById('iem'));
		_update();
	};

	var IemDisplay = React.createClass({displayName: 'IemDisplay',
		getInitialState: function() {
			return { data: [] };
		},
		handleIemData: function() {

		},
		render: function() {
			var iemItems = this.state.data.items.map({

			});

			return (
				React.createElement("div", null, 
					React.createElement("ol", null, iemItems)
				)
			);
		}
	});
